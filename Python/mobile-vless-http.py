...
def parse_proxy_link(link: str) -> dict | None:
    ...
    # 1. Проверка security (должно быть пустой строкой или "none")
    security = params.get("security", ["none"])[0].lower()
    if security not in ["", "none"]:
        return None

    # 2. Проверка encryption (должно быть "none")
    encryption = params.get("encryption", ["none"])[0].lower()
    if encryption != "none":
        return None

    # 3. Проверка типа сети (должен быть строго "tcp")
    net_type = params.get("type", ["tcp"])[0].lower()
    if net_type != "tcp":
        return None

    # 4. Проверка headerType (должен быть строго "http")
    header_type = params.get("headerType", [""])[0].lower()
    if header_type != "http":
        return None

    # 5. Извлечение UUID (user)
